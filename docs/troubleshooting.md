...
**Symptom:** openclaw starts without logging `[policy-engine] Plugin activated`.
...
2. Confirm the build output exists:
   ```bash
   ls ~/.openclaw/plugins/policy-engine/dist/index.js
   ```
...
1. Confirm the watcher started — look for `[policy-engine] Watching src/policy/rules.ts` in the openclaw log.
2. The watcher debounces by 300 ms. Wait a moment after saving.
3. Check the log for a reload error:
   ```
   [policy-engine] Failed to reload rules: SyntaxError: ...
   ```
   If present, fix the syntax error in `rules.ts` and save again.
...
1. Confirm the `AUDIT_LOG_FILE` path is set correctly and the file exists.
2. The ABAC engine logs audit entries only when an `AuditLogger` is passed to `PolicyEngine`. Verify the engine was constructed with `{ auditLogger }`.
3. The Cedar-style engine (lifecycle hooks) does not currently write to the audit file. Entries are generated by the ABAC engine or posted directly via `POST /api/audit`.
...
The Cedar-style engine uses **forbid-wins** semantics. If **any** `forbid` rule matches, access is denied regardless of matching `permit` rules. Order does not matter — unlike priority-based ABAC engines.
